...
- Use `Rail` from `github.com/curtisnewbie/miso/flow` for all logging
- Use `errs` from `github.com/curtisnewbie/miso/errs` for all error handling
- Use `dbquery.NewQuery()` from `github.com/curtisnewbie/miso/middleware/dbquery` for database operations
- Get DB instances via `mysql.GetMySQL()` or `sqlite.GetSQLite()` middleware
- Define configuration properties as constants with `misoconfig-*` tags